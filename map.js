...
const createResponsivePopupContent = (destination) => {
  if (isMobileMapViewport()) {
    return `
      <article class="map-popup map-popup-mobile">
        <h3>${escapeHtml(destination.name)}</h3>
        <div class="map-popup-meta">
          <span>${escapeHtml(destination.category)}</span>
        </div>
  ...
  return createPopupContent(destination);
};
...
const buildCompactSidebarItem = (destination) => `
  <button class="map-list-item${selectedDestinationId === destination.id ? " is-active" : ""}" type="button" data-destination-id="${escapeHtml(destination.id)}">
    <span class="map-list-row">
      <span class="map-list-category">${escapeHtml(destination.category)}</span>
...
const buildCompactSelectedCardContent = (destination) => `
  <article class="map-selected-content">
    <div class="map-selected-head">
      <div class="map-selected-copy">
        <div class="map-selected-chip-row">
          <span class="map-list-category">${escapeHtml(destination.category)}</span>
          <span class="map-list-pill"><i class="fa-solid fa-star" aria-hidden="true"></i> ${escapeHtml(destination.ratingLabel)}</span>
          <span class="map-selected-chip"><i class="fa-solid fa-ticket" aria-hidden="true"></i> ${escapeHtml(destination.price)}</span>
        </div>
        <h3>${escapeHtml(destination.name)}</h3>
        <p class="map-selected-district"><i class="fa-solid fa-location-dot" aria-hidden="true"></i> ${escapeHtml(destination.district)}</p>
      </div>
      <button class="map-selected-dismiss" type="button" aria-label="Tutup detail destinasi" data-dismiss-selected>
        <i class="fa-solid fa-xmark" aria-hidden="true"></i>
      </button>
...
const updateSelectedCard = () => {
  ...
  mapSelectedCard.innerHTML = isMobileMapViewport()
    ? buildCompactSelectedCardContent(destination)
    : buildSelectedCardContent(destination);
...
const updateSidebar = () => {
  mapDestinationList.innerHTML = filteredDestinations
    .map(isMobileMapViewport() ? buildCompactSidebarItem : buildSidebarItem)
    .join("");
...
const updateMarkers = () => {
  ...
  filteredDestinations.forEach((destination) => {
    ...
    marker.bindPopup(createResponsivePopupContent(destination), {
      maxWidth: 288,
